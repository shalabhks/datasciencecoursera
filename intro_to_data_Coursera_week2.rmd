...
xx <- rdu_flights$arr_delay
hist(x=xx,freq=FALSE,col="blue",breaks = 24)
```
...
nycflights %>% group_by(tailnum) %>% arrange(desc(avg_speed)) %>% select(tailnum,avg_speed)
```
...
nycflights %>% group_by(dep_type) %>% summarise(on_time_p=sum(arr_type == "on time")/n())
